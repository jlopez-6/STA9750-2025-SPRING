...
The analysis in this project was completed with R and selected packages^[@baseR; @readr; @dplyr; @stringr; @rvest; @httr; @jsonlite; @lubridate; @tidyr; @janitor; @purrr; @tidyverse; @car; @randomForest; @scales], and with Python and selected packages^[@python; @pandas; @requests; @beautifulsoup4; @pathlib].
...
x_vars <- c("per_capita_gdp", "unemployment_rate", "per_capita_personal_income", "per_capita_property_tax_revenues",
            "per_capita_annual_debt_service", "dividends_proportion", "farm_proportion", "retirement_proportion",
            "wage_salary_proportion", "ui_comp_proportion", "proprietors_income_proportion")
...
x_labels <- c("Per Capita GDP", "Unemployment Rate", "Per Capita Personal Income", "Per Capita Property Tax Revenues",
              ...
              "Wage & Salary Proportion", "UI Compensation Proportion", "Proprietors' Income Proportion")
...
# Generating a discrete viridis color palette, one color per county
...
# Create dropdown buttons for selecting x variable
buttons <- lapply(seq_along(x_vars), function(i) {
  ...
  list(
    ...
    args = list(
      list(
        x = x_data,
        marker = list(color = county_colors)  # keep colors consistent on update
      ),
...
The plots above reveal clear patterns in how different economic indicators relate to median home sale prices. For example, indicators like GDP, per capita personal income, dividend proportion, and proprietors' income proportion show a strong positive correlation with home prices, suggesting wealthier or more economically active counties tend to have higher home values. On the other hand, indicators such as wage & salary proportion, UI compensation proportion, and unemployment rate appear to have little to no clear correlation with home prices. Farm proportion and retirement proportion exhibit weak negative correlations, suggesting that counties with larger agricultural or retiree populations may have slightly lower median home prices.

## Comparison of Economic Indicators Across Counties
```{r, eval = TRUE}
...
pretty_labels <- c("Per Capita GDP", "Unemployment Rate", "Per Capita Personal Income",
                   ...
                   "Wage & Salary Proportion", "UI Compensation Proportion",
                   "Proprietors' Income Proportion")
names(pretty_labels) <- indicators

# Prep my data
plot_data <- CT_SALES_ECONOMY |>
  select(county, year_recorded, all_of(indicators)) |>
  pivot_longer(cols = all_of(indicators), names_to = "indicator", values_to = "value")

years <- sort(unique(plot_data$year_recorded))

# add frames
frames <- list()
for (yr in years) {
  for (ind in indicators) {
    frame_name <- paste(yr, ind, sep = "_")
    frame_data <- plot_data |>
      filter(year_recorded == yr, indicator == ind) |>
      arrange(value)
    frames[[frame_name]] <- frame_data
  }
}

# Start with first frame data
init_year <- years[1]
init_indicator <- indicators[1]
init_frame <- frames[[paste(init_year, init_indicator, sep = "_")]]

counties <- init_frame$county
...
fig <- plot_ly(
  ...
  layout(
    title = paste0("Indicator: ", pretty_labels[init_indicator], " in Year ", init_year),
    xaxis = list(title = "Value"),
    yaxis = list(title = "County", categoryorder = "total ascending"),
    margin = list(l = 100, r = 50, t = 100, b = 100)
  )

# Helper to generate x and y data for a specific year and indicator
get_xy <- function(yr, ind) {
  df <- frames[[paste(yr, ind, sep = "_")]]
  list(x = df$value, y = df$county)
}

# Dropdown for years
year_buttons <- lapply(years, function(yr) {
  list(
    method = "restyle",
    args = list(
      list(
        x = lapply(indicators, function(ind) frames[[paste(yr, ind, sep = "_")]]$value)[[1]],
        y = list(counties)
      )
    ),
...
indicator_buttons <- lapply(indicators, function(ind) {
  list(
    method = "restyle",
    args = list(
      list(
        x = list(frames[[paste(init_year, ind, sep = "_")]]$value),
        y = list(counties)
      )
    ),
    label = pretty_labels[ind],
    execute = TRUE
  )
})
...
year_buttons <- lapply(years, function(yr) {
  list(
    ...
    args = list(
      list(
        x = list(frames[[paste(yr, init_indicator, sep = "_")]]$value),
        y = list(counties)
      ),
      list(
        title = paste0("Indicator: ", pretty_labels[init_indicator], " in Year ", yr)
      )
    ),
    label = as.character(yr)
  )
...
indicator_buttons <- lapply(indicators, function(ind) {
  list(
    ...
    args = list(
      ...
      list(
        title = paste0(pretty_labels[ind], " in Year ", init_year)
      )
    ),
    label = pretty_labels[ind]
  )
...
fig
```
Fairfield tops nearly all economic indicators, including the less favorable ones, which strongly suggests that population size is the primary driver of these differences. Larger populations tend to amplify both positive and negative metrics simply due to scale. This population effect underscores how county size can influence economic and housing market indicators, making it important to consider population when comparing counties.
...
plot_rf_results <- function(predictions, actuals, save_dir = file.path("figs",RESPONSE)) {
  ...
  # Save plots to the specified directory with descriptive names
...
run_rf_pipeline <- function(data) {
  ...
  # Choose the best model: prioritize high var_explained and fewest predictors
  ...
  # --- Partial Dependence Plot for retirement proportion ---
  
  # List of variables you want to include in the grid
  top_vars <- rownames(as.data.frame(importance(rf_results$model, type = 1)))
  
  # Create PDPs for each variable and combine them
