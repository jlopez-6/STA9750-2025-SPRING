...
plot <- ggplot() +
  ...
  labs(color = "Shift",
       title = "Voter County-level Shift between 2020 and 2024",
       caption = "Source: Wikipedia Presidential Election Results Data.")
...
load_election_results <- function(states, year){
  ...
  if (!file.exists(dest_fpath)){
    ...
    if (year == 2020){
      # consolidate misc parties vote counts into other parties
      election_results <- 
        election_results |>
        select(-c(totalvotescast, totalvotes, registeredvoters, voterturnout, total)) |>
        mutate(
...
```
Here, I locate the state with the smallest shift toward Trump in 2024.
...
election_results |>
  group_by(state) |> 
  summarize(state_republican_count_change = sum(republican_count_change, na.rm = TRUE)) |>
  slice_min(state_republican_count_change, n = 1) |>
  rename(
    "State" = state,
    "Change in Votes" = state_republican_count_change
  ) |>
...
```
Finally, I deduce the county that had the largest increase in voter turnout in the most recent election. 
