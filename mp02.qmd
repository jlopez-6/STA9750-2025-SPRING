...
GTA IV's green score was calculated from the geometric mean[^1] of agency emissions per capita (EPC) and agency emissions per mile (EPM)--with lower green scores meaning greener agencies. When asked about the win, TriMet’s representative proudly declared above the roaring crowd: "We do it for our riders--because they can’t take trips if they’ve died of air pollution."

[^1]: The geometric mean was chosen because EPC and EPM are on different scales.
...
ggplot(green_agencies_plot, aes(y = fct_reorder(agency, -green_score), x = green_score)) +
  ...
  scale_x_continuous(breaks = seq(0, 2, by = 0.5)) +  # Fix x-axis tick spacing for readability
  theme_minimal() +
  theme(
...
``` {r emissions_avoided, eval=TRUE,echo=FALSE, warning=FALSE, message=FALSE, include=TRUE}
...
bubble_data <- agency_modes |> 
  ...
  arrange(desc(emissions_avoided))  # Sort for annotation
...
``` {r emissions-avoided-plot, eval=TRUE, echo=FALSE, warning=FALSE, message=FALSE, include=TRUE}
...
``` {r green-giant, eval=TRUE, echo=FALSE, warning=FALSE, message=FALSE, include=TRUE}
...
top_statal <- 
  ...
  ungroup()
```
``` {r green-giant-plot, eval=TRUE, echo=FALSE, warning=FALSE, message=FALSE, include=TRUE}
...
Because natural gas was the most "dirty" fuel type in this data set, GTA IV identified agencies deserving the Most Noxious Rating by as having the largest percentage of their total emissions from natural gas. The table below presents the small-, medium-, and large-sized agencies with the highest percentage of emissions from natural gas.
``` {r noxious, warning=FALSE, message=FALSE, echo=FALSE, eval=TRUE, include=TRUE}
...
ggplot(combined_pie_data, aes(x = "", y = pct_of_emissions, fill = fuel_type)) +
  ...
  coord_polar(theta = "y") +
  scale_fill_brewer(palette = "Set2") +  # Standard color scheme
  labs(
    x = "", y = "", fill = "Fuel Type",
